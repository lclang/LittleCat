...
open class NumberClass(name: String = "number", val number: Number = 0): LCClass(name) {
    init {
        ...
        globals["toInt"] = method(returnType = Types.INT) { IntClass.get(number.toInt()) }
        ...
        globals["toChar"] = method(returnType = Types.CHAR) { CharClass.get(number.toChar()) }
    }
    ...
    operator fun plus(numberClass: NumberClass): NumberClass =
        when{
            ...
            else -> IntClass.get(number.toInt()+numberClass.number.toInt())
        }
    ...
    fun larger(numberClass: NumberClass) = BoolClass.get(number.toDouble() > numberClass.number.toDouble())
    fun largerEquals(numberClass: NumberClass) = BoolClass.get(number.toDouble() >= numberClass.number.toDouble())
    fun less(numberClass: NumberClass) = BoolClass.get(number.toDouble() < numberClass.number.toDouble())
    fun lessEquals(numberClass: NumberClass) = BoolClass.get(number.toDouble() <= numberClass.number.toDouble())
}