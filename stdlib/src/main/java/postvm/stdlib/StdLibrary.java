...
//        globals.put("LC_BUILD", IntClass.get(Global.buildTime));
        ...
        globals.put("exit", voidMethod((caller, args) -> System.exit(args.get(0).cast(IntClass.class).value.intValue()),
                IntClass.TYPE));
        globals.put("sleep", voidMethod((caller, args) -> {
                    try {
                        Thread.sleep(args.get(0).cast(LongClass.class).value.intValue());
    ...
    public static class StdClass extends LibraryClass {
        ...
        private StdClass() {
            ...
            globals.put("getClassesCount", method((caller, args) -> IntClass.get(PostVMClass.classesCount), IntClass.TYPE));
